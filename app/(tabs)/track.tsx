...
function getWeeksWaited(dateStr: string): number {
  ...
  return Math.ceil((new Date().getTime() - referred.getTime()) / (1000 * 60 * 60 * 24 * 7));
}

function getProgressWidth(dateStr: string): `${number}%` {
  if (!dateStr) return '0%';
  const percent = Math.min(Math.round((getWeeksWaited(dateStr) / 18) * 100), 100);
  return `${percent}%`;
}
...
function getSteps(referralDate: string, hospital: string) {
  ...
  const weeksWaited = getWeeksWaited(referralDate);
...
export default function TrackScreen() {
    ...
    useCallback(() => {
      async function load() {
        ...
        if (stored) {
          ...
          // Migrate from old single-referral format
          ...
          if (spec) {
            const migrated: Referral[] = [{
              id: Date.now().toString(),
              specialty: spec,
              hospital: hosp || '',
              referralDate: date || '',
            }];
            setReferrals(migrated);
            setExpandedId(migrated[0].id);
  ...
  async function saveReferrals(updated: Referral[]) {
    setReferrals(updated);
    await AsyncStorage.setItem('user_referrals', JSON.stringify(updated));
  }
  ...
  function deleteReferral(id: string, specialty: string) {
    Alert.alert(
      `Remove ${specialty}?`,
      'This referral will be permanently removed.',
      [
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingTop: insets.top + 16 }}>
      ...
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.subtitle}>
            {referrals.length > 0
              ? `${referrals.length} active · Last updated today`
              : 'No active referrals'}
          </Text>
        </View>
          ...
          <View key={ref.id} style={styles.referralCard}>
            ...
            >
              <View style={{ flex: 1, marginRight: 10 }}>
                ...
                <Text style={styles.cardSub}>
                  {ref.referralDate || ref.hospital
                    ? `${ref.referralDate}${ref.hospital ? ' · ' + ref.hospital : ''}`
                    : 'No date or hospital set'}
                </Text>
              </View>
            ...
            <View style={styles.progressInfo}>
              <Text style={styles.progressLabel}>
                {ref.referralDate
                  ? getWeeksWaited(ref.referralDate) > 18
                    ? `Overdue by ${getWeeksWaited(ref.referralDate) - 18} weeks`
                    : `Week ${getWeeksWaited(ref.referralDate)} of 18`
                  : ''}
              </Text>
              <Text style={styles.progressLabel}>
                {ref.referralDate ? `${Math.max(0, 18 - getWeeksWaited(ref.referralDate))} weeks remaining` : ''}
              </Text>
            </View>
            ...
            {isExpanded && (
              <>
                ...
                <View style={styles.detailGrid}>
                  {[
                    { label: 'REFERRED', value: ref.referralDate || 'Unknown', color: '#1C1C1E' },
                    {
                      label: 'DEADLINE',
                      value: ref.referralDate
                        ? new Date(parseDate(ref.referralDate).getTime() + 18 * 7 * 24 * 60 * 60 * 1000)
                          .toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })
                        : 'Not set',
                      color: '#1C1C1E',
                    },
                    { label: 'TRUST', value: ref.hospital || 'Your trust', color: '#1C1C1E' },
                ...
                {/* Chaser + Delete buttons */}
                <View style={styles.cardActions}>
                  <TouchableOpacity
                    style={styles.chaserBtn}
                    onPress={() => Alert.alert(
                      'Send chaser letter?',
                      `This will generate a formal letter to ${ref.hospital || 'your hospital'} requesting an update on your ${ref.specialty} referral.`,
                      [
                        { text: 'Cancel', style: 'cancel' },
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => deleteReferral(ref.id, ref.specialty)}
                  >
                    <Feather name="trash-2" size={14} color="#A32D2D" />
                  </TouchableOpacity>
                </View>
      ...
      {/* ADD MORE — shown at bottom when referrals exist and under limit */}
      {referrals.length > 0 && referrals.length < MAX_REFERRALS && (
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <ScrollView style={styles.modal} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.modalSection}>
              ...
              <View style={styles.pillGrid}>
                {SPECIALTY_NAMES.map(s => (
                  <TouchableOpacity
                    key={s}
                    style={[styles.pill, newSpecialty === s && styles.pillActive]}
                    onPress={() => setNewSpecialty(s)}
                  >
            ...
            <View style={styles.modalSection}>
              ...
              >
                <Text style={{ color: newReferralDate ? '#1C1C1E' : '#C7C7CC', fontSize: 15 }}>
                ...
                <DateTimePicker
                  ...
                  onChange={(event, date) => {
                    ...
                    if (date) {
                      ...
                      setNewReferralDate(date.toLocaleDateString('en-GB', {
              ...
              {newReferralDate ? (
                <TouchableOpacity onPress={() => setNewReferralDate('')}>
                  <Text style={{ color: '#A32D2D', fontSize: 12, marginTop: 6 }}>Clear date</Text>
                </TouchableOpacity>
              ) : null}
            </View>
            ...
            {/* Save button */}
            <TouchableOpacity
              style={[styles.saveBtn, !newSpecialty && styles.saveBtnDisabled]}
              onPress={confirmAdd}
...
const styles = StyleSheet.create({
  ...
  linePending: { backgroundColor: '#E5E5EA' },
  stepText: { paddingBottom: 16, flex: 1 },
  stepLabel: { fontSize: 13, fontWeight: '500', color: '#1C1C1E', marginBottom: 2 },
  stepLabelPending: { fontWeight: '400', color: '#8E8E93' },
  stepDate: { fontSize: 11, color: '#8E8E93' },
  ...
  emptyTitle: { fontSize: 17, fontWeight: '500', color: '#1C1C1E', marginBottom: 8 },
  emptyBody: { fontSize: 13, color: '#8E8E93', textAlign: 'center', lineHeight: 20, marginBottom: 20 },
  ...
  pillText: { fontSize: 13, color: '#3C3C43' },
